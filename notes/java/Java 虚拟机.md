...
与CPU相关：记录正在执行的虚拟机字节码指令的地址（如果正在执行的是本地方法则为空）。

## Java 虚拟机栈（stack）

每个 Java 方法在执行的同时会创建一个栈帧用于存储局部变量(方法的参数或内部变量)、操作数栈、常量池引用等信息。从方法调用直至执行完成的过程（即方法的运行一定在栈当中），对应着一个栈帧在 Java 虚拟机栈中入栈和出栈的过程。**一旦超出作用域，立即从栈内存中消失。**
...
本地方法栈与 Java 虚拟机栈类似，它们之间的区别只不过是本地方法栈为本地方法服务，而是与操作系统相关。
...
所有对象（new出来的）都在这里分配内存，是垃圾收集的主要区域（"GC 堆"），堆内存的数据均有默认值。
...
存储.class相关的信息，包含方法的信息：用于存放已被加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。（方法的运行一定要在栈当中）
...
<div align="center"> <img src="../../pics/9422c251-2676-4f4b-8b3b-d67f2e9daffc.png" width="800px"> </div><br>数组内存示意图

<div align="center"> <img src="../../pics/62af9019-9b6f-4596-ad4b-880f33e9dd75.png" width="800px"> </div><br>只有一个对象的内存示意图

<div align="center"> <img src="../../pics/44cbdf34-b624-4075-8c90-2cfd3ee90198.png" width="800px"> </div><br>两个引用指向同一个对象的内存示意图

当使用一个对象类型作为方法的返回值时：返回值其实就是对象的地址。

当一个对象作为参数，传递党方法中时，实际传递的是对象的地址值。
